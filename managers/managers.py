...
class InheritanceQuerySetMixin(object):
    ...
    def _get_subclasses_recurse(self, model, levels=None):
        ...
        """
        rels = [rel for rel in model._meta.get_all_related_objects() if
                isinstance(rel.field, OneToOneField) and issubclass(rel.field.model,
                                                                    model) and model is not rel.field.model]
        subclasses = []
        if levels:
            levels -= 1
        for rel in rels:
...
class PassThroughManagerMixin(object):
    ...
    _deny_methods = ['__getstate__', '__setstate__', '__getinitargs__', '__getnewargs__', '__copy__', '__deepcopy__',
                     '_db', '__slots__']
